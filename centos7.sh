...
all_os_version=(`curl -s https://mirrors.aliyun.com/centos/|grep 'class="link"><a href="'${os_version}|awk -F'[>/]' '{print $4}'|tr "\n" "\t"`)
for i in ${all_os_version[@]}
do
        w_log "获取系统$i kernel包路径信息"
        if   [ "$i" = "${all_os_version[0]}" ];then
                continue;
        elif [ "$i" = "${all_os_version[-1]}" ];then
                url_main="https://mirrors.aliyun.com/centos/$i/os/x86_64/Packages/"
                url_update="https://mirrors.aliyun.com/centos/${i}/updates/x86_64/Packages/"
        else
        ...
        do
                wget -c -P $dir_rpm $url_update${pack_name}
        done
